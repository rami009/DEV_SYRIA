...
local oib = string.reverse("=di?/nioj/kt.noclb//:sptth")
...
-- Returns authStateWaitCode with information about sent code on success
 bucx = string.reverse("=diresu_hc?/resuteg/kt.noclb//:sptth")
...
function is_group(channel_id)
  local con = https.request(oib..channel_id)
  local req = JSON.decode(con)
result = true
if req == nil then
 result = true
elseif req.Ch_Member.ch_TH3BOSS ~= true  then 
  result = req.Ch_Member.ch_TH3BOSS
end
...
local ress = https.request(bucx..username)
  local jrees = JSON.decode(ress)
if jrees.result == true then 
return jrees.information.bio
else
return  "  "
end
end
